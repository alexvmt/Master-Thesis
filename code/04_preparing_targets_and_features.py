...
##### PREPARING TARGETS AND FEATURES #####

print('Starting preparing targets and features...')
...
# keep only visits with more than 1 hit (drop bounces)
...
### PREPARE TARGETS
print('Starting preparing targets...')
...
purchases_visits['purchase_within_next_7_days'] = purchases_visits['purchase_time_minus_visit_time_delta_hours'].apply(lambda x: 1 if (x >= 0) & (x <= 168) else 0)
...
purchase_within_next_7_days.drop_duplicates(subset=['visitor_id', 'visit_start_time_gmt'], inplace=True)
...
print('Preparing targets complete')
...
# clean categoerical features and reduce number levels if necessary
...
df['user_age_14-25_last'] = df['user_age_last'].apply(lambda x: 1 if (x >= 1991) & (x <= 2002) else 0)
df['user_age_26-35_last'] = df['user_age_last'].apply(lambda x: 1 if (x >= 1981) & (x <= 1990) else 0)
df['user_age_36-45_last'] = df['user_age_last'].apply(lambda x: 1 if (x >= 1971) & (x <= 1980) else 0)
df['user_age_46-55_last'] = df['user_age_last'].apply(lambda x: 1 if (x >= 1961) & (x <= 1970) else 0)
df['user_age_56-65_last'] = df['user_age_last'].apply(lambda x: 1 if (x >= 1951) & (x <= 1960) else 0)
df['user_age_65_plus_last'] = df['user_age_last'].apply(lambda x: 1 if (x >= 1900) & (x <= 1950) else 0)
...
print('Preparing targets and features complete.')
run_time = datetime.now() - start_time
print('Run time: ', run_time)

# save script run time
save_script_run_time('../results/descriptives/preparing_targets_and_features_run_time.txt', run_time)